...
export default function RevealScreen({
  ...
  async function handleReveal() {
    ...
    try {
      if (userRole === "creator") {
        const joinerTerms = await revealTermsVault(joinerVaultUuid, joinerEncryptedData, walletClient, publicClient) as Terms;
        setTheirTerms(joinerTerms);
      } else {
        const creatorTerms = await revealTermsVault(creatorVaultUuid, creatorEncryptedData, walletClient, publicClient) as Terms;
        ...
        <div className="gloss-panel w-full px-6 py-8 flex flex-col items-center gap-5 text-center">
          ...
          {error && (
            <div className="flex flex-col gap-3 w-full gloss-panel border border-red-800/40 bg-red-900/10 px-4 py-3 text-left">
              ...
              {isRetryable && (
                <button onClick={handleReveal} className="self-start rounded-md bg-red-800/30 px-3 py-1.5 text-xs font-medium text-red-300 hover:bg-red-700/40 transition-colors">
                  Try Again
                </button>
              )}
            </div>
...
>
  {displayCount}
      ...
      {myTerms && theirTerms && alignedCount < 5 && (
        <div className="gloss-panel p-4 flex flex-col gap-3">
